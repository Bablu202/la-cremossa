...
export default function ProductCard({ product, onOrder }: ProductCardProps) {
  return (
    <div className="flex flex-col bg-[var(--bg-secondary)] shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 border-[0.5px] border-[var(--text-muted)]/20 group cursor-pointer w-full max-w-[360px] mx-auto rounded-xl">
      ...
      <div className="relative w-full h-80 bg-[#f8f6f0] rounded-t-xl overflow-hidden flex items-center justify-center">
        <Image 
          src={product.image} 
          alt={product.name} 
          fill
      ...
      <div className="p-6 flex flex-col w-full">
        <div>
          ...
          <p className="text-[var(--text-muted)] text-xs mb-3 tracking-widest uppercase">{product.servings} Servings</p>
          <span className="block text-3xl font-light text-[var(--price-color)] mb-4">{product.price}€</span>
        </div>
